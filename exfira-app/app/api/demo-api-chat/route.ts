...
const PYTHON_SERVICE_URL = process.env.PYTHON_CHAT_SERVICE_URL ?? "http://localhost:8001";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId) {
      console.warn("[/api/demo-api-chat] no session user — conversations will not be saved");
    }
    ...
    // ── Forward to Python (ChatGPT redaction) ──────────────────────────────
    ...
    console.error("[/api/demo-api-chat]", err);
