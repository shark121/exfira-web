...
export default function CompliancePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (filterPii) params.set("pii", filterPii);
  ...
  function applySearch() { setPage(1); setSearch(searchInput); }
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg, #f5f5f7)", fontFamily: "var(--font-body, Inter, sans-serif)" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={{
          ...
          <select
            value={filterRisk}
            onChange={e => { setPage(1); setFilterRisk(e.target.value); }}
          ...
          <select
            value={filterPii}
            onChange={e => { setPage(1); setFilterPii(e.target.value); }}
            style={{ fontSize: 12, borderRadius: 7, border: "0.5px solid rgba(0,0,0,0.15)", padding: "5px 10px", background: "transparent", cursor: "pointer", color: "var(--on-surface, #1d1d1f)" }}
          >
        ...
        {loading && events.length === 0 ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 0", color: "var(--outline, #8e8e93)", fontSize: 13 }}>No events found.</div>
        ) : (
