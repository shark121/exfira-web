...
app = FastAPI(title="Exfira PII Redaction Service (Gemma 4)", lifespan=lifespan)
...
def health():
    return {
        "status": "ok",
        "detector": "gemma4",
        "model": os.getenv("GEMMA_MODEL", "gemma4:2b"),
    }
...
async def redact_and_chat(req: ChatRequest, request: Request):
    ...
    # ── Step 1: Detect (Gemma 4 via Ollama) ───────────────────────────────────
    ...
    if vault:
        ...
        logger.info(
            "   Detection  : %.1fms  —  %d entit%s found",
            detection_ms,
            len(vault),
            "y" if len(vault) == 1 else "ies",
        )
        for token, (_, original, entity_type) in vault.items():
            logger.info("     %-30s  →  %s  (%s)", original, token, entity_type)
    else:
        logger.info(
            "   Detection  : %.1fms  —  no PII found, prompt sent as-is", detection_ms
        )
    ...
    logger.info(
        "◀  Done  total=%.1fms  (detection=%.1fms  llm=%.1fms)",
        total_ms,
        detection_ms,
        llm_ms,
    )
